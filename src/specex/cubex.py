...
def __argshandler(options: Optional[list[str]] = None) -> Any:
    ...
    parser.add_argument(
        ...
        'within this ellipse will be added together to compute the spectra. '
    ...
    parser.add_argument(
        ...
        'column name used to read the aforementioned quantities are '
    ...
    parser.add_argument(
        ...
        'The name of the region will be used as ID for the spectra.'
    )

    parser.add_argument(
        '--nspectra', metavar='N', type=int, default=0,
        help='Number of spectra to extract (useful for debugging purposes).'
    )
    ...
    parser.add_argument(
        ...
        "extracted spectrum with linearly interpolated data. In this case the "
    ...
    parser.add_argument(
        ...
        'NOTE that this value is zero indexed (i.e. firsts HDU has index 0).'
    )
    ...
    parser.add_argument(
        ...
        ". Note that in order to use external images as weightmap, the python "
...
def load_specex_file(
    filename: str | os.PathLike[str],
    spec_hdu_index: Optional[int]=None,
    var_hdu_index: Optional[int]=None,
     wd_hdu_index: Optional[int]=None,
    mask_hdu_index: Optional[int]=None,
    smoothing: int=0
):
    """
...
def speclist2ez(
        ...
        spec_hdu_name: str='SPECTRUM',
        var_hdu_name:str='VARIANCE'
):
    """
...
def specex(options: Optional[list[str]] = None) -> None:
    """
    ...
    if args.catalog is not None:
        catalog_path: Path = Path(args.catalog)

        if not catalog_path.is_file(follow_symlinks=True):
            print(f"The catalog '{catalog_path}' does not exits!")
            sys.exit(1)

        if args.cat_format is not None:
            sources = Table.read(catalog_path, format=args.cat_format)
        else:
            try:
                sources = Table.read(catalog_path)
            except IORegistryError:
                try:
                    sources = Table.read(catalog_path, format='ascii')
                except IORegistryError:
                    print(
        ...
        if not HAS_REGION:
            ...
            sys.exit(1)
        regionfile_path: Path = Path(args.regionfile)

        if not regionfile_path.is_file(follow_symlinks=True):
            print(f"The region file '{regionfile_path}' does not exits!")
            sys.exit(1)
    ...
    cube_path: Path = Path(args.input_cube)
    if not cube_path.is_file(follow_symlinks=True):
        logging.error(
            f"The cube '{cube_path}' does not exist!"
        )
        sys.exit(1)

    with SpectralCube.open(cube_path) as my_cube:
        if my_cube.spec_hdu is None:
            logging.error(
                f"Input cube has no spectral HDU! "
            )
            sys.exit(1)
        ...
        weight_img: None | np.ndarray = None
